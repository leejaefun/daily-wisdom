...
async function runDeepRiskAudit() {
  ...
  assert(versionTs.includes('APP_VERSION = "1.1.6"'), 'VER-01', 'Constants APP_VERSION is "1.1.6"');
  assert(versionTs.includes('BUILD_NUMBER = "8"'), 'VER-02', 'Constants BUILD_NUMBER is "8"');
  assert(pkgJson.includes('"version": "1.1.6"'), 'VER-03', 'package.json version is "1.1.6"');
  assert(pbxproj.includes('MARKETING_VERSION = 1.1.6;'), 'VER-04', 'Xcode MARKETING_VERSION is 1.1.6');
  assert(pbxproj.includes('CURRENT_PROJECT_VERSION = 8;'), 'VER-05', 'Xcode CURRENT_PROJECT_VERSION is 8');
  ...
  try {
    ...
    const seVersionText = await page.evaluate(() => document.body.innerText.includes('Daily Wisdom v1.1.6'));
    assert(seVersionText, 'E2E-SE-01', 'iPhone SE viewport renders "Daily Wisdom v1.1.6" footer text');
    ...
    await page.goto(`${BASE_URL}/settings`, { waitUntil: 'networkidle2' });
    await page.evaluate(() => new Promise(r => setTimeout(r, 400)));
    const dynamicIslandText = await page.evaluate(() => document.body.innerText.includes('Daily Wisdom v1.1.6'));
    assert(dynamicIslandText, 'E2E-17-01', 'iPhone 17 Pro viewport renders "Daily Wisdom v1.1.6" footer text');
